...
const initialForm = {
  ...
  totalArea: '',
  partialArea: '',
  estimatedCost: '', // Optional estimated cost
  latitude: '',
  longitude: '',
};
...
export function CrisisReportForm({ onSubmit, apiPathPrefix = '/data-encoder' }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (formData.estimatedCost) formPayload.append('estimatedCost', formData.estimatedCost); // Optional estimated cost
      ...
      const endpoint = `${API_URL}${apiPathPrefix}/crisis-reports`;
      ...
      if (!response.ok) {
        setMessage({ type: 'error', text: data.error || data.message || 'Failed to submit crisis report' });
        return;
      }

      setMessage({ type: 'success', text: 'Crisis report submitted successfully! ✓' });
      ...
      setMessage({ type: 'error', text: 'Failed to submit crisis report. Please try again.' });
    } finally {
      setLoading(false);
    }
  };
