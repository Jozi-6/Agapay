...
function DAInterventions() {
  ...
  const fetchDAInterventions = async () => {
    try {
      setLoading(true);
      ...
      const url = `${API_URL}/admin/da-interventions${params.toString() ? '?' + params.toString() : ''}`;
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch DA interventions');
      }
      ...
      setError('Failed to load DA interventions');
      setBeneficiaries([]);
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-600">
          Showing <span className="font-semibold text-gray-800">{beneficiaries.length}</span> DA intervention beneficiaries
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600 mb-4">No DA intervention beneficiaries found.</p>
          {hasActiveFilters && (
            <button
              onClick={clearFilters}
